 ...
 * such, there is no way of defining methods {@code read} or {@code print} methods suitable for all
 * possible clients.
 *
 * The default {@link #eval(String)} implementation uses an {@link ICommandInvoker} to process user
 * input. This {@link ICommandInvoker} invokes {@link IReplCommand}s, which use
...
public interface IRepl {
     ...
     */
    default IHook eval(String input)
        throws MetaborgException, CommandNotFoundException {
        input = input.trim();
        if (input.length() == 0) {
            return null;
        }
